import Image from 'next/image';
import type { TestimonialFields } from '@/types/contentful';
import { getImageUrl, getImageAlt } from '@/types/contentful';

interface TestimonialCardProps {
  testimonial: {
    fields: TestimonialFields;
  };
}

export default function TestimonialCard({ testimonial }: TestimonialCardProps) {
  const { customerName, company, position, content, rating, avatar } = testimonial.fields;
  const avatarUrl = getImageUrl(avatar);
  const avatarAlt = getImageAlt(avatar, customerName);

  return (
    <div className="border-2 border-neon-green/50 rounded-lg p-6 bg-dark-card">
      <div className="flex items-center mb-4">
        {avatarUrl ? (
          <Image
            src={avatarUrl}
            alt={avatarAlt}
            width={48}
            height={48}
            className="rounded-full mr-4"
          />
        ) : (
          <div className="w-12 h-12 rounded-full bg-neon-green/20 flex items-center justify-center mr-4">
            <span className="text-neon-green font-bold text-xl">
              {customerName.charAt(0)}
            </span>
          </div>
        )}
        <div>
          <h3 className="text-white font-semibold">{customerName}</h3>
          {(company || position) && (
            <p className="text-gray-400 text-sm">
              {position && company ? `${position} at ${company}` : position || company}
            </p>
          )}
        </div>
      </div>
      <div className="flex mb-3">
        {[...Array(5)].map((_, i) => (
          <svg
            key={i}
            className={`w-5 h-5 ${i < rating ? 'text-neon-green' : 'text-gray-600'}`}
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
        ))}
      </div>
      <p className="text-gray-300 italic">&quot;{content}&quot;</p>
    </div>
  );
}
